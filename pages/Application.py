...
with tf.device('/CPU:0'):
    gc_conv = tf.keras.models.load_model(DATA_DIR / 'models' / 'gradcam_conv.hdf5') # gradcam (conv)
    gc_clf = tf.keras.models.load_model(DATA_DIR / 'models' / 'gradcam_clf.hdf5') # gradcam (clf)

with tf.device('/CPU:0'):
    unet_cnn = tf.keras.models.load_model(DATA_DIR / 'models' / 'unet_256.hdf5') # unet

# Titre 
# Title
title_1,title_2,title_3 = st.columns([2,3,2])
title_2.markdown("<p style='padding: 10px; border: 2px solid white;text-align: center;font-size: 20px;'>Application</p>", unsafe_allow_html=True)
title_2.write("Séléctionnez ou importez une image, et essayez les fonctions disponibles.")
